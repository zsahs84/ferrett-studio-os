...
<body><h1>${lastExport.title}</h1><pre>${lastExport.text.replace(/</g, '&lt;')}</pre></body></html>`;
        ...
        const cmdkActions = [
            ...
            { icon: '🗂️', label: 'Song board', keys: 'board kanban status songs tracks', run: () => { window.switchTab('toolbox'); window.openTbxPanel?.('arrange'); } },
            { icon: '🔬', label: 'Mix analyzer', keys: 'lufs loudness true peak spectrum phase dynamic range analyze', run: () => { window.switchTab('toolbox'); window.openTbxPanel?.('analyzer'); } },
