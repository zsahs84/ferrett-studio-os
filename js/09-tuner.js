(function(){
  ...
  // verb drives the shared progress bar's label (window.startAiProgress, js/02-app-core.js) — same
  // bar as Producer Notes / Lyria Prompt Generator / Lyrics Lab, replacing the old plain spinner.
  // These four Toolbox tools are short single replies, so a 12s fill is enough for a typical call.
  async function brainCall(sys,user,creative,verb){ lastBrainArgs=[sys,user,creative,verb]; if(!window.ferrettAI){ brainShow('AI not available.'); return; } /* same pre-Gemini gate as the Lyria one — see the note there; this one silently disabled every Toolbox AI tool in gemini mode */ if(!(window.__aiIsConfigured?.())){ if(window.__openAiModal) window.__openAiModal(); brainShow('Set up the AI connection first (⚙ setup).'); return; } const progress=window.startAiProgress('ai-brain-progress-wrap','ai-brain-progress-bar','ai-brain-progress-label', verb||'Thinking', 12000); brainShow(''); try{ window.__aiUsage?.begin('Toolbox'); const r=await window.ferrettAI(sys,user,{creative}); window.__aiUsage?.end(); progress.stop(true); brainShow(r); }catch(e){ progress.stop(false); brainShow('⚠ '+e.message); } }
  ...
  function aiAsk(){ const q=$('ai-ask-in').value.trim(); if(!q) return; brainCall('You are a veteran recording/mixing engineer and producer. Answer concisely and practically — a few sentences or tight bullets, real moves not fluff.'+gearContext(), q, false, 'Answering'); }
  function aiMood(){ const q=$('ai-mood-in').value.trim(); if(!q) return; brainCall('You suggest chord progressions for songwriters. Give 2-3 progressions as chord symbols (e.g. Am–F–C–G), each with a short why. No preamble.', 'Mood: '+q, true, 'Suggesting chords'); }
  function aiRecipe(){ const sel=$('ai-recipe-sel'); const id=+sel.value; const r=((window.db&&window.db.cookbook)||[]).find(x=>x.id===id); if(!r){ brainShow('Pick a recipe.'); return; } const genre=$('ai-recipe-genre').value.trim()||'a different genre'; brainCall('You are a music production expert. Reimagine the given production recipe for a new genre. Be practical and specific: key sonic moves, plugin/gear analogs, arrangement notes. Keep it tight.'+gearContext(), `Original (${r.genre} — ${r.inst}): ${r.desc||''} ${r.reaper||''}\n\nReimagine for: ${genre}.`, true, 'Reimagining the recipe'); }
  function aiMix(){ const q=$('ai-mix-in').value.trim(); if(!q) return; brainCall('You are a mixing engineer. Given described mix problems, return a short prioritized fix list — each item a concrete move with frequency/dB/tool. No preamble.'+gearContext(), q, false, 'Working the mix fixes'); }
